...
This library updates an [earlier version](https://github.com/kbose28/FarmTest) that implements the **f**actor-**a**djusted **r**obust **m**ultiple **test**ing (FarmTest) method proposed by [Fan et al., 2019](https://www.tandfonline.com/doi/full/10.1080/01621459.2018.1527700). To explicitly caputre the strong dependency among features, we assume the *d*-dimensional data vectors *X<sub>i</sub>* follow a factor model *X<sub>i</sub> = &mu; + Bf<sub>i</sub> + &epsilon;<sub>i</sub>*, where *f<sub>i</sub>* are the common factors, *B* denotes the factor loading matrix, and *&epsilon;<sub>i</sub>* are idiosyncratic errors. Assume that *f* and *&epsilon;* are independent and have zero means so that *&mu;* is the mean vector. We are interested in testing the hypothesis *&mu; = &mu;<sub>0</sub>*. We assume the data is of dimension *p* with sample size *n*, leading to *p* hypothesis tests. 
